...
const Whatsapplayer = () => {
    ...
    if (token) {
      try {
        ...
        if (payload.role === 'reseller' && userData?.id) {
          ...
          setError("Only resellers can configure WhatsApp settings");
          setFetching(false);
        }
      } catch (err) {
  ...
  const fetchWhatsappConfig = async (resellerId) => {
    setFetching(true);
    setError("");
    try {
      const result = await getMstWhatsappConfigByResellerId(resellerId);
  ...
  const handleSubmit = async (e) => {
    ...
    const existingConfig = await getMstWhatsappConfigByResellerId(resellerId);
    ...
    setLoading(true);
    try {
      const result = await upsertMstWhatsappConfig(resellerId, {
          ...
          <div className='row'>
            ...
            <div className='d-flex align-items-center justify-content-center gap-3 mt-24'>
              <button
                ...
                onClick={() => {
                  fetchWhatsappConfig(resellerId);
                  setError("");
                  setSuccess("");
                }}
                disabled={loading}
