...
import { getMstSmtpTemplatesByResellerId, createMstSmtpTemplate, updateMstSmtpTemplate, deleteMstSmtpTemplate } from "@/hasura/mutations/smtpTemplate";
...
const EmailTemplates = () => {
    ...
    if (token) {
      try {
        ...
        if (payload.role === 'reseller' && userData?.id) {
          ...
          setError("Only resellers can manage SMTP templates");
          setFetching(false);
        }
      } catch (err) {
  ...
  const fetchTemplates = async (resellerId) => {
    ...
    setError("");
    try {
      const result = await getMstSmtpTemplatesByResellerId(resellerId);
      if (result.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        result = await createMstSmtpTemplate(resellerId, {
          template_name: form.template_name.trim(),
          template_type: form.template_type,
          subject: form.subject.trim(),
          body: form.body.trim(),
            ...
            <div className='row'>
              ...
              <div className='col-sm-12'>
                <div className='mb-20'>
                  ...
                  <textarea
                    ...
                    rows='10'
                    className='form-control radius-8'
                    placeholder={`Hello {{name}},\n\nYour OTP is {{otp}}\n\nOr HTML: <p>Hello {{name}},</p><p>Your OTP is {{otp}}</p>`}
                    value={form.body}
                    onChange={handleChange}
                    disabled={loading}
                    autoComplete="off"
                  ...
                  <small className='text-secondary'>
                    You can use plain text or HTML format. Use variables like <b>{`{{name}}`}</b>, <b>{`{{otp}}`}</b>
                  </small>
                </div>
              </div>
              ...
              <div className='col-sm-12'>
                <div className='mb-20'>
                  ...
                  />
                  <small className='text-secondary'>
                    Use variables like <b>{`{{name}}`}</b>, <b>{`{{otp}}`}</b>
                  </small>
                </div>
              </div>
