...
const AdminSMTPLayer = () => {
    ...
    if (token) {
      try {
        ...
        if ((payload.role === 'admin' || payload.role === 'super_admin') && userData?.id) {
          ...
          setError("Only admins can configure SMTP settings");
          setFetching(false);
        }
      } catch (err) {
  ...
  const fetchSmtpConfig = async (adminId) => {
    setFetching(true);
    setError("");
    try {
      const result = await getMstSmtpConfigByAdminId(adminId);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await upsertMstSmtpConfigByAdminId(adminId, {
        host: form.host.trim(),
        port: port,
