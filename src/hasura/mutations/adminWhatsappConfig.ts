...
/**
 * Get WhatsApp config by admin ID
 */
export const getMstWhatsappConfigByAdminId = async (adminId: string) => {
  ...
  const QUERY = `query GetMstWhatsappConfigByAdminId($admin_id: uuid!) {
    mst_whatsapp_config(where: { admin_id: { _eq: $admin_id } }, limit: 1) {
      id
      admin_id
      api_key
      api_url
...
/**
 * Create or update WhatsApp config for admin
 */
export const upsertMstWhatsappConfigByAdminId = async (
  adminId: string,
  data: {
    api_key: string;
    api_url: string;
  ...
  const checkResult = await getMstWhatsappConfigByAdminId(adminId);
  ...
  if (checkResult.success && checkResult.data) {
    ...
    const UPDATE_MUTATION = hasApiKey
      ? `mutation UpdateMstWhatsappConfigByAdminId(
          $id: uuid!
          $api_key: String!
          $api_url: String!
          $phone_number_id: String!
    ...
    // Create new
    const INSERT_MUTATION = `mutation InsertMstWhatsappConfigByAdminId(
      $admin_id: uuid!
      $api_key: String!
      $api_url: String!
      $phone_number_id: String!
