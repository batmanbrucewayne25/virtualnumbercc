...
export const getMstResellerDomainByResellerId = async (resellerId: string) => {
  ...
  try {
    ...
    if (
      result?.data?.mst_reseller_domain &&
      result.data.mst_reseller_domain.length > 0
    ) {
      return {
        success: true,
  ...
  try {
    ...
    if (result?.errors) {
      return {
        success: false,
        message:
          result.errors[0]?.message || "Failed to fetch reseller by domain",
        data: null,
      };
    }
    if (
...
const getDomainOwnerResellerId = async (
  normalizedDomain: string,
): Promise<string | null> => {
  const domainWithoutWww = normalizedDomain.replace(/^www\./, "");

  const QUERY = `query GetDomainOwner($domain1: String!, $domain2: String!) {
    mst_reseller_domain(
      where: {
        _or: [
          { domain: { _eq: $domain1 } },
  ...
  if (result?.errors || !result?.data?.mst_reseller_domain?.length) {
    return null;
  }

  const record = result.data.mst_reseller_domain[0];
  return record.reseller;
};
...
export const upsertMstResellerDomain = async (
  ...
  if (!domain || typeof domain !== "string" || domain.trim() === "") {
    console.warn("upsertMstResellerDomain: Domain is empty or invalid", {
      domain,
      resellerId,
    });
    return {
  ...
  console.log("upsertMstResellerDomain called:", {
    resellerId,
    domain,
    normalizedDomain,
  });
  ...
  try {
    // Check if reseller already has a domain (needed for update vs insert logic)
    ...
    // Unique constraint: domain cannot be assigned to multiple resellers
    const ownerResellerId = await getDomainOwnerResellerId(normalizedDomain);
    if (ownerResellerId && ownerResellerId !== resellerId) {
      return {
        success: false,
        message:
          "This domain is already assigned to another reseller. Each domain must be unique.",
      };
    }

    // Reseller already has a domain - check if it's the same (no-op) or different (update)
    ...
    if (existingResult.success && existingResult.data) {
      ...
      if (adminId && uuidRegex.test(adminId)) {
        ...
        console.log(
          "Inserting new domain WITHOUT approvedBy (reseller self-update)",
        );
      }
    }
    ...
    console.log(
      "Mutation type:",
      existingResult.success && existingResult.data ? "UPDATE" : "INSERT",
    );
    ...
    try {
      ...
      if (domainRecord) {
        console.log(
          "✅ Domain record saved/updated successfully:",
          domainRecord,
        );
      ...
      if (result?.data) {
        ...
        if (result.data.insert_mst_reseller_domain_one === null) {
          console.error(
            "⚠️ insert_mst_reseller_domain_one returned NULL - might be a permissions issue",
          );
        }
        if (result.data.update_mst_reseller_domain_by_pk === null) {
          console.error(
            "⚠️ update_mst_reseller_domain_by_pk returned NULL - might be a permissions issue",
          );
        }
      }
      ...
      return {
        success: false,
        message:
          "Failed to save domain - no record returned. Check console for details.",
        data: result?.data,
        fullResult: result,
      };
    } catch (error: any) {
  ...
  try {
    ...
    const domainsWithResellerData = await Promise.all(
      domains.map(async (domainRecord) => {
        ...
        try {
          const resellerResult = await graphqlRequest(RESELLER_QUERY, {
          ...
          console.warn(
            `Failed to fetch reseller ${domainRecord.reseller}:`,
            err,
          );
          return { ...domainRecord, reseller_data: null };
        }
